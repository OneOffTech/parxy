...
from parxy_core.drivers import PyMuPDF4LLMDriver
from parxy_core.exceptions import FileNotFoundException


class TestPyMuPDF4LLMDriver:
    ...
    def test_pymupdf4llm_driver_can_be_created(self):
        driver = PyMuPDF4LLMDriver()

        assert driver.supported_levels == ['page', 'block']

    def test_pymupdf4llm_driver_unrecognized_level_handled(self):
        driver = PyMuPDF4LLMDriver()
    ...
    def test_pymupdf4llm_driver_handle_not_existing_file(self):
        driver = PyMuPDF4LLMDriver()
    ...
    def test_pymupdf4llm_driver_read_empty_document_page_level(self):
        driver = PyMuPDF4LLMDriver()
    ...
    def test_pymupdf4llm_driver_read_document(self):
        driver = PyMuPDF4LLMDriver()
    ...
    def test_pymupdf4llm_driver_tracing_span_created(self, mock_tracer):
        mock_span = MagicMock()
        ...
        assert doc_processing_call[1]['driver'] == 'PyMuPDF4LLMDriver'
        ...
        assert count_call[1]['driver'] == 'PyMuPDF4LLMDriver'

    @patch('parxy_core.drivers.abstract_driver.tracer')
    def test_pymupdf4llm_driver_tracing_exception_recorded(self, mock_tracer):
        mock_span = MagicMock()
        mock_span.__enter__ = Mock(return_value=mock_span)
        mock_span.__exit__ = Mock(return_value=False)
        mock_tracer.span = Mock(return_value=mock_span)
    ...
    def test_pymupdf4llm_driver_records_elapsed_time(self):
        driver = PyMuPDF4LLMDriver()
