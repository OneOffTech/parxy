...
class TestUnstructuredLocalDriver:
    ...
    def test_unstructured_local_driver_tracing_span_created(self, mock_tracer):
        ...
        assert doc_processing_call[1]['driver'] == 'UnstructuredLocalDriver'
        ...
        assert count_call[1]['driver'] == 'UnstructuredLocalDriver'

    @patch('parxy_core.drivers.abstract_driver.tracer')
    def test_unstructured_local_driver_tracing_exception_recorded(self, mock_tracer):
        # Setup mocks
        mock_span = MagicMock()
        mock_span.__enter__ = Mock(return_value=mock_span)
        mock_span.__exit__ = Mock(return_value=False)
