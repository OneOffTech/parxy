...
class TestPyPDFiumfDriver:
    ...
    def test_pypdfium_driver_can_be_created(self):
        driver = PyPDFium2Driver()

        assert driver.supported_levels == ['page', 'block']

    def test_pypdfium_driver_unrecognized_level_handled(self):
        driver = PyPDFium2Driver()
    ...
    def test_pypdfium_driver_handle_not_existing_file(self):
        driver = PyPDFium2Driver()
    ...
    def test_pypdfium_driver_read_empty_document_page_level(self):
        driver = PyPDFium2Driver()
    ...
    def test_pypdfium_driver_read_document(self):
        driver = PyPDFium2Driver()
    ...
    def test_pypdfium_driver_tracing_span_created(self, mock_tracer):
        ...
        assert doc_processing_call[1]['driver'] == 'PyPDFium2Driver'
        ...
        assert count_call[1]['driver'] == 'PyPDFium2Driver'

    @patch('parxy_core.drivers.abstract_driver.tracer')
    def test_pypdfium_driver_tracing_exception_recorded(self, mock_tracer):
        # Setup mocks
        mock_span = MagicMock()
        mock_span.__enter__ = Mock(return_value=mock_span)
        mock_span.__exit__ = Mock(return_value=False)
    ...
    def test_pypdfium_driver_reads_outline(self):
        driver = PyPDFium2Driver()
        ...
        assert isinstance(entry.bbox, BoundingBox)
        assert entry.bbox.x0 == 69.0
        assert entry.bbox.y0 == 629.0

    def test_pypdfium_driver_records_elapsed_time(self):
        driver = PyPDFium2Driver()
